...
UAuraAttributeSet::UAuraAttributeSet()
{
	...
	/**
	 * Following code is to initialize mapping tags to gameplay attribute
	 * Hard code
	 **/
...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	/*
	 * Specify which properties need to call its RepNotify function
	 * when the property is replicated from server
	 * 
	 * Each properties required to have form of following example.
	 * ** UPROPERTY(BlueprintReadOnly, ReplicatedUsing=OnRep_MaxHealth) **
	 * where ReplicatedUsing is used to specify the RepNotify function name
	 * which is declared and defined.
	 */
...
void UAuraAttributeSet::HandleImcomingXP(const FEffectProperties& Props)
{
	const float LocalIncomingXP = GetIncomingXP();
	SetIncomingXP(0.f);
		
	// Source character is owner, since GA_ListenForEvents apply GE_EventBasedEffect
		...
		&& Props.SourceCharacter->Implements<UCombatInterface>())
	{
		...
		if (NumLevelUps > 0)
		{
			int32 FinalAttributePointsReward = 0;
			int32 FinalSpellPointsReward = 0;
			int32 FinalLevel = 0;
			...
			for (int32 i = CurrentLevel; i < NewLevel; i++)
			{
				...
				FinalAttributePointsReward += AttributePointsReward;
				
				const int32 SpellPointsReward = IPlayerInterface::Execute_GetSpellPointsReward(
					Props.SourceCharacter,
					i
				);
				FinalSpellPointsReward += SpellPointsReward;
			...
			// Add to player level
			IPlayerInterface::Execute_AddToPlayerLevel(Props.SourceCharacter, FinalLevel);
				
			// Add to attribute and spell points
			IPlayerInterface::Execute_AddToAttributePoints(
				Props.SourceCharacter,
				FinalAttributePointsReward
			);
			IPlayerInterface::Execute_AddToSpellPoints(
				Props.SourceCharacter,
				FinalSpellPointsReward
			);
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, const float Damage, bool bBlockedHit, bool bCriticalHit) const
{
	if (Props.SourceCharacter != Props.TargetCharacter)
	{
		// Tell client side who cause damage to show damage number
		// only if source character is player controlled and
		// target character is enemy(AuraAIController)
		...
		// Tell client side who cause damage to show damage number
		// only if source character is enemy(AIController) and
		// target character is player controlled(AuraPlayerController)
